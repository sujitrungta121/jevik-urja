...
class Option extends CI_Controller {
   ...
   public function detail(){
	...
	$data["language_id"] 	= $this->uri->segment(5);
 	...
 	$data["unit_list"]=$this->option_model->unit_list();
	...
	public function add_new_value(){
		...
		$data["unit_list"]=$this->option_model->unit_list();
      	...
      	if($_POST){
			$add_value = false;
			if($this->option_model->check_value_exist($_POST)>0){
				$this->session->set_flashdata('action_message', 'Same Option Value Name Already Exist.. Unable To Add');
				$this->session->set_flashdata('action_message_type', 'danger');
				redirect($_SERVER['HTTP_REFERER']);
				exit();
			}
  ...
  public function delete(){
   ...
   public function value_update(){
 	$this->load->model('admin/option_model');
      	...
      	if($_POST){
			$update_value = false;
			$found=$this->option_model->check_bulk_value_exist($_POST);
			if($found>0){
				$this->session->set_flashdata('action_message', $found.' Duplicate Value Found... Unable To Update ');
				$this->session->set_flashdata('action_message_type', 'danger');
				redirect($_SERVER['HTTP_REFERER']);
				exit();
			}
   ...
   public function delete_value(){
 	$this->load->model('admin/Option_model','om');
			...
			$delete_value = false;
			$delete_value = $this->om->delete($this->uri->segment(4));
			if($delete_value){
