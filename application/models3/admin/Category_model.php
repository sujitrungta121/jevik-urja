...
class Category_model extends CI_Model {
	...
	public function get_categories($start, $limit){
			// of Opencart
			$query = $this->db->query("SELECT cp.category_id AS category_id, GROUP_CONCAT(cd1.category_name ORDER BY cp.level SEPARATOR ' &gt; ') AS category_name, c.parent_id, c.rank, c.main_image,c.banner_image
										...
										WHERE cd1.language_id = '2' AND cd2.language_id = '2' GROUP BY cp.category_id ORDER BY category_name
										LIMIT ".$start.", ".$limit."");
			$result= $query->result();
	...
	public function Add($data){
		$this->db->trans_begin();
			...
			$this->db->insert("category",array("parent_id"=>$data['parent_id'],"rank"=>$data['rank'],"main_image"=>$data["main_image"],"banner_image"=>$data["banner_image"],"small_image"=>$data["banner_image"]));
			/*if($main_update){
				return true;
			}*/
			...
			foreach ($data['category_description'] as $language_id => $value) {
				///as english language is used in url /////////////////
				if($language_id=="2"){
					$slug_name=seoString($value['category_name']);
				}
			...
			$this->db->update("category",array("slug_name"=>$slug_name),array("id"=>$category_id),1);
	...
	public function delete($data){
			...
			$main_update = $this->db->query("DELETE FROM category WHERE id = '" . (int)$data . "'");
			$main_update = $this->db->query("DELETE FROM category_description WHERE category_id = '" . (int)$data . "'");
			if($main_update){
				return true;
			}
	...
	public function update($category_id, $data){ 
		$this->db->trans_begin();
	// of Opencart
			/*$main_update = $this->db->query("UPDATE category SET parent_id = '" . (int)$data['parent_id'] . "', rank = '" . (int)$data['rank'] . "' WHERE id = '" . (int)$category_id . "'");*/
			$main_tbl_update_fields=array("parent_id"=>$data["parent_id"]);
			if(isset($data["small_image"])){
				$main_tbl_update_fields["small_image"]=$data["small_image"];
			}
			if(isset($data["main_image"])){
				$main_tbl_update_fields["main_image"]=$data["main_image"];
			}
			if(isset($data["banner_image"])){
				$main_tbl_update_fields["banner_image"]=$data["banner_image"];
			}
			$this->db->update("category",$main_tbl_update_fields,array("id"=>$category_id),1);
			//echo $this->db->last_query(); exit();
