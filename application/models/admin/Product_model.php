...
function get_products(){
        ...
        if(!empty($this->session->userdata('user_brand_id'))){
        	$this->db->where("p.brand_id",$this->session->userdata('user_brand_id'));
        }
 ...
 public function update($product_id, $data){ // of Opencart
 	...
 	$this->db->trans_begin();
		...
		$pr_tb_update_fields=array(
			...
			"wb_old_price"=>$data["wb_old_price"],
			"up_price"=>$data["up_price"],
			"up_old_price"=>$data["up_old_price"],
			"rank"=>$data["rank"]);
		if($data["image"]!=""){
