...
class Api_model extends CI_Model{
    ...
    function save_registration_data($name,$email,$mobile,$password,$referral_code,$gst_no,$state_id,$time,$fcm,$id=""){
        $this->db->trans_begin();
        if($id==""){
            $own_referral_code=$this->create_referral_code($name);
            $this->db->insert("user",array("name"=>$name,"email"=>$email,"mobile"=>$mobile,"password"=>$password,"created_on"=>$time,"own_referral_code"=>$own_referral_code,"referral_code"=>$referral_code,"status"=>3,"user_type_id"=>9,"state_id"=>$state_id,"gst_no"=>$gst_no));
...
*/
    ...
    function product_list($args=array()){
       ...
       $this->db->select("cd.category_name,bd.name as brand_name,cd.category_id,p.wb_price,p.up_price,p.wb_old_price,p.up_old_price,pov.wb_price as variant_price,pov.old_price as variant_old_price,p.slug_name,p.id as product_id,p.stock,p.show_unit_in,p.base_unit_value_stock,ov.base_unit_value,pov.stock,p.image,pd.name,ov.value_name as selected_option_name,pov.id as selected_option_id,pov.disc");
        ...
        $this->db->join("product_description pd","pd.product_id=p.id","inner");
         ...
         $this->db->join("brand_description bd","bd.brand_id=p.brand_id and cd.language_id=2","left");
    ...
    function addToCart($product_id,$variant_id,$qty,$user_id,$operator="+",$unit="TON"){
        //WE ARE taking ton as standard unit 
        $this->db->trans_begin();
        //$qty=1.5;
        if($this->cartDataExist($user_id)==0){
            ...
            if($this->cartDetailsDataExist($id,$product_id,$qty,$variant_id,strtoupper($unit))>0){
                //echo "exist "; exit();
                $option_details=$this->product_option_details($variant_id);
                if($unit=="KG"){
                   $unit_price=$option_details['base_price']+$option_details["base_price"]/1000;
                ...
                if ($operator === "=") {
                    ...
                    $this->db->set("total_price", "unit_price * ".$qty, FALSE);
                    $this->db->set("total_old_price", "old_price * ".$qty, FALSE);
                } else {
                    $this->db->set("count","count".$operator.$qty,FALSE);
                    $this->db->set("total_price","total_price".$operator.$unit_price,FALSE);
                    $this->db->set("total_old_price","total_old_price".$operator.$old_price,FALSE);
                }
    ...
    function cartDetailsData($user_id){
        $this->db->select("p.image,od.product_id,pd.name as product_name,od.unit_price as price,od.total_price,od.old_price,od.count,od.options as variant_id,ov.value_name as variant_name,od.oid,od.unit");
       /* $this->db->select("p.image,od.product_id,pd.name as product_name,od.unit_price as price,pov.price,pov.old_price,od.count,od.options as variant_id,ov.value_name as variant_name,od.oid,od.unit");*/
        $this->db->from("order_detail od");
    ...
    function cartDetailsDataExist($order_id,$product_id,$qty,$variant_id,$unit="TON"){
        $this->db->select("count(order_id) as found");
        $this->db->from("order_detail");
        $this->db->where("order_id",$order_id);
        $this->db->where("product_id",$product_id);
    ...
    function getOrders($user_id){
        ...
        foreach($order_data as $ord){
            ...
            od.count,od.unit_price,od.total_price,pd.name as product_name,od.options as variant_id,ov.value_name as variant_name,od.oid,od.unit ");
            $this->db->from("order_detail od");
            $this->db->join("product p","p.id=od.product_id","inner");
            $this->db->join("product_description pd","pd.product_id=od.product_id and pd.language_id=2","inner");
            $this->db->join("product_option_value pov","pov.id=od.options","inner");
    ...
    function verify_forgot_password_otp($otp,$email){
        $this->db->select("count(user_id) as found");
        $this->db->from("user");
        $this->db->where(array("fp_otp"=>$otp,"email"=>$email));
        $this->db->where("status",1);
        return  $this->db->get()->row()->found;
    }
