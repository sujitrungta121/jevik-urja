...
class Api extends CI_Controller{
	...
	function registration($purpose="form-submit"){
		...
		$id=$this->am->save_registration_data($name,$email,$mobile_no,$password,$referral_code,$gst_no,$state_id,$time,$fcm);
		if($id>0){
			...
			$config = array(
			    'protocol'  => 'smtp',
			    'smtp_host' => SMTP_HOST,       // e.g., 'ssl://smtp.gmail.com' or 'smtp.yourprovider.com'
			    'smtp_port' => SMTP_PORT,       // e.g., 465 (SSL) or 587 (TLS)
			    'smtp_user' => SMTP_USER,
			...
			$this->email->from(SMTP_SENDER_ID, 'Jevik Urja');

			$admin_subject = "New User Registration Done";

			$admin_message = $this->load->view('email_template/registration_notification_to_business', [
			    'user' => (object)[
			        'name' => $name,
			        'email' => $email,
			        'phone' => $mobile_no,
			...
			$this->email->to(SITE_OWNERS_DEFAULT_EMAIL_ID);
			...
			$this->email->clear(TRUE); // Use TRUE to clear attachments & all

			$welcome_subject = "Welcome to Jevik Urja, {$name}!";
			$welcome_message = $this->load->view('email_template/user_welcome_notification', [
			    'user' => (object)[
			        'name' => $name,
			        'email' => $email,
			    ],
			    'site_name' => 'Jevik Urja',
			    'site_url' => base_url(),
			    'login_url' => base_url('login')
			], true);

			$this->email->from(SMTP_SENDER_ID, 'Jevik Urja'); // Use same sender ID here too
			$this->email->to($email);
			$this->email->subject($welcome_subject);
	...
	function productDetails(){
    	...
    	/*echo "<pre>";
    	print_r($details); exit();*/
    	$details["related_products"]=$this->am->product_list(array("category_id"=>$details["category_id"],"exclude_product_id"=>$details["product_id"],"limit"=>20,"offset"=>0));
	...
	public function send() {
        ...
        if ($this->email->send()) {
            ...
            echo '❌ Failed to send email.<br><pre>';
	...
	function login(){
		...
		if($user_name=='' || $password==''){
			echo json_encode(array("status"=>"failed","status_code"=>0,"msg"=>"You need to fill-up login information properly"));
			exit();
		}
		if($this->am->login_data_exist($user_name)==1){
			...
			if($authenticated){
				if($user_data['status']=='3'){
					echo json_encode(array("status"=>"failed","status_code"=>0,"msg"=>"your account is not approve please contact +91 983003 3408 for approval."));
					exit();
				}
	...
	function completeOrder($source="web"){
        ...
        if ($order_id) {
		    ...
		    $invoice_url = base_url('Api/bill/' . $order_id); // Replace if different

		    // === (1) Email to Business Admin ==
		    $admin_subject = "New Order Received - #" . $order_id;
		    $admin_message = $this->load->view('email_template/order_placed_notification_to_business', [
		        'order' => (object)[
		            'order_id' => $order_id,
		            'created_at' => date("Y-m-d H:i:s"),
		            'customer_name' => $order_details['customer_name'],
		            'customer_email' => $order_details['email'],
		            'customer_phone' => $order_details['mobile'],
		            'invoice_url' => $invoice_url
		        ]
		    ], true);

		    $this->email->from(SMTP_SENDER_ID, SITE_TITLE);
		    $this->email->to(SITE_OWNERS_DEFAULT_EMAIL_ID);
		    $this->email->subject($admin_subject);
		    $this->email->message($admin_message);
		    $this->email->send();
		    ...
		    $user_message = $this->load->view('email_template/order_placed_notification_to_user', [
		        'order' => (object)[
		            'order_id' => $order_id,
		            'created_at' => date("Y-m-d H:i:s"),
		            'customer_name' => $order_details['customer_name'],
    ...
    $exist = $this->am->email_exist($number);
    if ($exist == 0) {
        echo json_encode(array("status" => "failed", "status_code" => 0, "msg" => "Email id Not Exist"));
        exit();
    }

    $user_details = $this->am->get_user_details_by_email($number);
    $email = $user_details['email'];

    if (empty($email)) {
        echo json_encode(array("status" => "failed", "status_code" => 0, "msg" => "Email not found for this user"));
        exit();
    }
    ...
    if ($updated) {
        ...
        $config = array(
            'protocol'  => 'smtp',
            'smtp_host' => SMTP_HOST,  // Use ssl:// prefix
            'smtp_port' => SMTP_PORT,
            'smtp_user' => SMTP_USER,
            'smtp_pass' => SMTP_PASSWORD,
            'mailtype'  => 'html',
        ...
        if ($this->email->send()) {
            echo json_encode(array("status" => "success", "status_code" => 1, "msg" => "OTP has been sent to your email: " . $email));
        } else {
            echo json_encode(array("status" => "failed", "status_code" => 0, "msg" => "Unable to send OTP email"));
        }
    } else {
        echo json_encode(array("status" => "failed", "status_code" => 0, "msg" => "Unable To Send OTP"));
    }
    ...
    function resetPassword(){
    	...
    	if($verified==0){
    		echo json_encode(array("status"=>"failed","status_code"=>0,"msg"=>"OTP or Email id mismatched"));
    		exit();
    	}else{
    		//$user_details=$this->am->get_user_details_by_mobile_no($mobile);
    		$user_details=$this->am->get_user_details_by_email($mobile);
...
*/
    ...
    function update_cart_total(){
    	$this->load->model("Api_model","am");
    	$grand_total=$this->am->update_order_total(294);
