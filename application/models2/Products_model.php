...
class Products_model extends CI_Model
{
    ...
    public function most_sell_products()
    {
        $query = $this->db->query('SELECT modules.*, product.*, product_description.*
			...
			WHERE product_description.language_id = 2 AND modules.type = "sell"
    ...
    public function most_popular_products()
    {
        $query = $this->db->query('SELECT modules.*, product.*, product_description.*
			...
			INNER JOIN product ON modules.product_id=product.id
			INNER JOIN product_description ON product.id=product_description.product_id
			WHERE product_description.language_id = 2 AND modules.type = "popular"
			ORDER BY modules.rank ASC');
        return $query->result();
    }
    ...
    public function slider_products()
    {
        $query = $this->db->query('SELECT modules.*, product.*, product_description.*
			...
			INNER JOIN product ON modules.product_id=product.id
			INNER JOIN product_description ON product.id=product_description.product_id
			WHERE product_description.language_id = 2 AND modules.type = "slide"
			ORDER BY modules.rank ASC');
        return $query->result();
    }
    public function category_products($id)
    {
        $query = $this->db->query('SELECT product.*, product_description.*
			FROM product
			INNER JOIN product_description ON product.id=product_description.product_id
			WHERE product_description.language_id = 2 AND product.category_id = '.$id.'
			ORDER BY product.id');
        return $query->result();
    }
    public function product($id)
    {
        $query = $this->db->query('SELECT product.*, product_description.*
			FROM product
			INNER JOIN product_description ON product.id=product_description.product_id
			WHERE product_description.language_id = 2 AND product.id = '.$id.'');
        $data= $query->result();
        //echo $this->db->last_query();exit();
        return $data;
